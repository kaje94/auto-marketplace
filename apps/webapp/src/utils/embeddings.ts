...
export const getTextEmbeddings = (listingItemData: PartialMessage<ListingItem_Data>, listingUser: PartialMessage<UserProfile>): number[] => {
    ...
    const embeddings: number[] = [
        tokenizer.encode(VEHICLE_BRANDS.findIndex((item) => item === listingItemData?.brand).toString()).pop() ?? 0,
        ...
        tokenizer.encode(listingItemData?.yearOfManufacture?.toString()!).pop() ?? 0,
        tokenizer.encode(listingItemData?.condition === VehicleConditionTypes.BrandNew ? "New" : listingItemData?.condition!).pop() ?? 0,
        tokenizer.encode(getVehicleTypeEmbed(listingItemData?.type!)).pop() ?? 0,
        tokenizer.encode(listingItemData?.transmissionType!).pop() ?? 0,
        tokenizer.encode(listingItemData?.fuelType!).pop() ?? 0,
        ...
        tokenizer.encode(listingUser?.data?.countryCode!).pop() ?? 0,
    ];
