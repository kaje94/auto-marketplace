...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    const brandsFetchResp = await fetch(`${env.API_BASE_URL}/v1/Vehicles/brands`);
    const brandsRespJson: { id: string; name: string }[] = await brandsFetchResp.json();

    for (const brand of brandsRespJson) {
        searchMaps.push({
            url: `${env.AUTH0_BASE_URL}/${BOT_LOCALE}/search?${queryString.stringify({ Brand: brand.name })}`,
